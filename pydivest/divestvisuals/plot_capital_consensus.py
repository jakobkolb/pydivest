...
ax3.plot(output_data[tstart:, 0], output_data[tstart:, 4], label=labels[4],
         zorder=2)
ax3.plot(output_data[tstart:, 0], output_data[tstart:, 5], label=labels[5],
         zorder=2)
ax3.plot(output_data[tstart:, 0],
         (output_data[tstart:, 4] + output_data[tstart:, 5]) / 2., zorder=2)
df = pd.DataFrame(index=output_data[tstart:, 0])
df['A'] = output_data[tstart:, 16]
mpbl = ax3.pcolorfast(ax3.get_xlim(), ax3.get_ylim(),
                      df['A'].values[np.newaxis], cmap='RdYlGn', alpha=.3,
