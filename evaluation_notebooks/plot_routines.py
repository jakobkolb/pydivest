...
class plot_routines(object):
    ...
    def __init__(self,
        ...
        self.data_sets = [
            self.micro_mean, self.micro_sem, self.macro_mean, self.macro_sem
        ]
        ...
        self.variable_combos = [['x', 'y', 'z'], ['c', 'g'],
                                ['mu_c^c', 'mu_d^d'], ['mu_c^d', 'mu_d^c']]
    ...
    def mk_plots(self, bd):
        ...
        fig = plt.figure()
        axes = [
            fig.add_subplot(l_vars, l_phi, i + 1 + j * l_phi)

            for i in range(l_phi) for j in range(l_vars)
        ]
    ...
    def mk_opinon_plots(self,
        ...
        for d in self.data_sets:
            local_datasets.append(
                d.xs(list(selector.values()), level=list(selector.keys())))
    ...
    def mk_4plots_v2(self,
        ...
        self.variable_combos = [['N_c over N', '[cc] over M', '[cd] over M'],
                                ...
                                ['K_c^c', 'K_d^d'],
                                ['K_c^d', 'K_d^c']]
        self.variable_combos_backup = [[
            'N_c over N', '[cc] over M', '[cd] over M'
        ], ['c', 'g'], ['K_c^c', 'K_d^d'], ['K_c^d', 'K_d^c']]
        ...
        for i in range(4):
            ...
            for k, variable in enumerate(variables):
                ...
                try:
                    ...
                    print(
                        f'Key Error for {self.variable_combos[j]} in dataset {k}'
                    )
