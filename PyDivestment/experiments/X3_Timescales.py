...
def RUN_FUNC(t_a, phi, eps, t_G, alpha, test, filename):
    ...
    """
    assert isinstance(test, types.IntType), 'test must be int, is {!r}'.format(
        test)
...
if getpass.getuser() == "kolb":
    SAVE_PATH_RAW = '/p/tmp/kolb/Divest_Experiments/divestdata/{}/raw_data_{}/'.format(
        folder, input_int)
    SAVE_PATH_RES = '/home/kolb/Divest_Experiments/divestdata/{}/raw_data_{}/'.format(
        folder, input_int)
elif getpass.getuser() == "jakob":
    SAVE_PATH_RAW = '/home/jakob/PhD/Project_Divestment/Implementation/divestdata/{}/raw_data_{}/'.format(
        folder, input_int)
    SAVE_PATH_RES = 'home/jakob/PhD/Project_Divestment/Implementation/divestdata/{}/raw_data_{}/'.format(
        folder, input_int)
...
EVA1 = {"<mean_trajectory>":
            lambda fnames: pd.concat(
                [np.load(f)["economic_trajectory"] for f in fnames]).groupby(
                level=0).mean(),
        "<sem_trajectory>":
            lambda fnames: pd.concat(
                [np.load(f)["economic_trajectory"] for f in fnames]).groupby(
...
EVA2 = {"<mean_convergence_state>":
            lambda fnames: np.nanmean(
                [np.load(f)["convergence_state"] for f in fnames]),
        "<mean_convergence_time>":
            lambda fnames: np.nanmean(
                [np.load(f)["convergence_time"] for f in fnames]),
        "<min_convergence_time>":
            lambda fnames: np.nanmin(
                [np.load(f)["convergence_time"] for f in fnames]),
        "<max_convergence_time>":
            lambda fnames: np.max(
                [np.load(f)["convergence_time"] for f in fnames]),
        "<nanmax_convergence_time>":
            lambda fnames: np.nanmax(
                [np.load(f)["convergence_time"] for f in fnames]),
...
if mode == 0:
    SAMPLE_SIZE = 100
    handle = experiment_handling(SAMPLE_SIZE, PARAM_COMBS, INDEX, SAVE_PATH_RAW,
                                 SAVE_PATH_RES)
