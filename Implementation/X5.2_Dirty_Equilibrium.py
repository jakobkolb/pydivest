...
def RUN_FUNC(t_a, phi, alpha,
    ...
    (N, p, tau, P, b_d, b_R0, e, s) =\
        (100, 0.125, 0.1, 500, 1.2, 1., 100, 0.23)
...
"""
FOLDER_EQUI = 'X5.2_Dirty_Equilibrium'
FOLDER_TRANS = 'X5.2_Dirty_Clean_Transition'
...
"""
t_as = [round(x, 5) for x in list(10**np.linspace(-1, 2, 4))]
...
"""
t_a, phi, alpha, t_d, test = [0.1], [0.8], [0.001], [30.], [0]
...
if mode == 1:  # Production
    PARAM_COMBS = list(it.product(
        t_as, phis, alphas, t_d, [opinion_presets], eps, transition, test))
    ...
    test = [True]
    t_as = [round(x, 5) for x in list(10**np.linspace(-1., 1., 3))]
    phis = [round(x, 2) for x in list(np.linspace(0.0, 1.0, 4))[:-1]]
    PARAM_COMBS = list(it.product(
            t_as, phis, alpha, t_d, [opinion_presets], eps, transition, test))
else:
